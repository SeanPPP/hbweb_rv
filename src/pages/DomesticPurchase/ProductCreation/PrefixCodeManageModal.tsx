...
export default function PrefixCodeManageModal({ visible, supplierCode, supplierName, onClose, onSuccess }: PrefixCodeManageModalProps) {
  ...
  const loadList = useCallback(async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        message.error(res.message || '加载前缀码失败')
      }
    } catch {
      setLoading(false)
      message.error('加载前缀码失败')
    }
  ...
  const columns: ColumnsType<PrefixCodeItem> = [
    ...
    },
    {
      ...
      render: (isActive, record) =>
        editingKey === record.prefixCode ? (
          ...
          <Switch checked={isActive} onChange={(checked) => handleToggleStatus(record.prefixCode, checked)} checkedChildren="启用" unCheckedChildren="停用" />
        ),
    },
    {
      title: '创建时间',
    ...
    },
    {
      ...
      render: (_, record) =>
        editingKey === record.prefixCode ? (
          <Space size="small">
            <Button type="link" size="small" onClick={handleEdit} loading={submitting}>确定</Button>
          ...
          <Space size="small">
            ...
            <Popconfirm title="确定删除？" onConfirm={() => handleDelete(record.prefixCode)}>
  ...
  return (
    <Modal title={`管理前缀码 - ${supplierName || supplierCode}`} open={visible} onCancel={onClose} width={900} footer={null} destroyOnClose>
      <Form form={form} layout="inline" style={{ marginBottom: 16 }}>
        <Row gutter={12} style={{ width: '100%' }}>
          <Col flex="120px">
            <Form.Item name="prefixName" rules={[{ required: true, message: '请输入前缀码' }, { pattern: /^[A-Za-z0-9]+$/, message: '仅限字母和数字' }]}>
              <Input placeholder="前缀码" maxLength={10} style={{ textTransform: 'uppercase' }} onChange={(e) => handlePrefixNameChange(e, form)} />
            </Form.Item>
          </Col>
          <Col flex="200px">
            <Form.Item name="prefixDescription"><Input placeholder="描述" /></Form.Item>
          </Col>
          <Col flex="80px">
          ...
          <Col>
            <Button type="primary" icon={<PlusOutlined />} onClick={handleAdd} loading={submitting}>添加</Button>
          </Col>
        </Row>
      </Form>
      <div style={{ marginBottom: 12 }}>
        <Space>
          <Input placeholder="搜索前缀码" prefix={<SearchOutlined />} value={search} onChange={(e) => setSearch(e.target.value)} onPressEnter={() => { setPage(1); loadList() }} style={{ width: 240 }} allowClear />
          <Button onClick={() => { setPage(1); loadList() }}>搜索</Button>
        </Space>
      </div>
      <Spin spinning={loading}>
        <Table columns={columns} dataSource={list} rowKey="prefixCode" size="small" pagination={{ current: page, pageSize, total, showSizeChanger: true, showQuickJumper: true, showTotal: (t) => `共 ${t} 条`, onChange: (p, ps) => { setPage(p); setPageSize(ps) } }} scroll={{ x: 800 }} />
      </Spin>
    </Modal>
  )
}
